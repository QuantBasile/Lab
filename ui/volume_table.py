...
class VolumeTable(ttk.Frame):
    """
    Pestaña 'volumen_tabla':
      - Filas = agrupación seleccionable (UND_NAME / CALL_OPTION / UND_TYPE / TYPE)
      - Columnas = ISSUER_NAME
      - Valores = Σ TXN_AMT
      - Modos: ABSOLUTO | POR_FILA_%
      - Totales opcionales (fila 'ALL' y columna 'ALL')
      - Export: CSV del contenido visible
      - Copiar al portapapeles (TSV)
      - Status: Dimensiones (filas × columnas) del contenido mostrado
    """

    GROUP_FIELDS = ("UND_NAME", "CALL_OPTION", "UND_TYPE", "TYPE")
    MODES = ("ABSOLUTO", "POR_FILA_%")
    ...
    def __init__(self, master=None):
        ...
        self._mode = tk.StringVar(value="ABSOLUTO")
    ...
    def _build(self):
        ...
        ttk.Label(top, text="Agrupar por:").grid(row=0, column=0, padx=(0, 6))
        cmb_group = ttk.Combobox(top, values=self.GROUP_FIELDS, textvariable=self._group_by,
                                 width=14, state="readonly")
        cmb_group.grid(row=0, column=1)
        cmb_group.bind("<<ComboboxSelected>>", lambda e: self._rebuild_and_refresh())

        ttk.Label(top, text="Vista:").grid(row=0, column=2, padx=(12, 6))
        cmb_mode = ttk.Combobox(top, values=self.MODES, textvariable=self._mode,
                                width=12, state="readonly")
        cmb_mode.grid(row=0, column=3)
        cmb_mode.bind("<<ComboboxSelected>>", lambda e: self._refresh_table())

        chk = ttk.Checkbutton(top, text="Incluir totales", variable=self._with_totals,
                              command=self._refresh_table)
        chk.grid(row=0, column=4, padx=(12, 0))

        # Acciones: Copiar / Export CSV (sin Excel)
        btn_copy = ttk.Button(top, text="Copiar", command=self._copy_to_clipboard)
        btn_csv = ttk.Button(top, text="Export CSV", command=self._export_csv)
        ...
        self._shape_var = tk.StringVar(value="Dimensiones: 0 × 0")
    ...
    def update_view(self, df: pd.DataFrame):
        """Recibe DF filtrado y actualiza la tabla."""
        self._df = df
    ...
    def _prepare_pivot(self):
        ...
        if grp_field == "UND_NAME":
            if "UND_NAME" in s.columns:
                ...
                s[idx_col_internal] = "(UND_NAME missing)"
                self._index_name = "UND_NAME"
        else:
            if grp_field not in s.columns:
                s[grp_field] = f"({grp_field} missing)"
    ...
    def _refresh_table(self):
        try:
            ...
            # Actualizar status de dimensiones (filas × columnas) tal y como se ve en pantalla
            r, c = df_show.shape  # incluye columna GROUP
            self._update_shape_label(r, c)

        except Exception as ex:
            messagebox.showerror("Error", f"No se pudo refrescar la tabla:\n{ex}")
    ...
    def _copy_to_clipboard(self):
        """Copia al portapapeles el contenido visible (TSV)."""
        if self._pivot_view is None or self._pivot_view.empty:
            messagebox.showinfo("Copiar", "No hay datos para copiar.")
            return
        df_out = self._pivot_view.reset_index().rename(columns={self._index_name: "GROUP"})
        try:
            ...
            self.update()
            messagebox.showinfo("Copiar", "Tabla copiada al portapapeles.")
        except Exception as ex:
            messagebox.showerror("Copiar", f"No se pudo copiar:\n{ex}")

    # ---------------- Helpers ----------------
    def _update_shape_label(self, rows: int, cols: int):
        self._shape_var.set(f"Dimensiones: {rows} × {cols}")
